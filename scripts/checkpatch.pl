...
sub help {
	...
	print << "EOM";
Usage: $P [OPTION]... [FILE]...
Version: $V (modified for nilfs-utils)
...
our $logFunctions = qr{(?x:
	v?(?:f|s|sn|)printf|
	(?:[a-z0-9]+_){1,2}(?:printf|logger)|
	v?syslog|
	perror|
	v?(?:err|warn)x?|
	die|error|perr|pinfo|myprintf|
)};
...
#line length limit
		if ($line =~ /^\+/ && $prevrawline !~ /\/\*\*/ &&
		    $rawline !~ /^.\s*\*\s*\@$Ident\s/ &&
		    !($line =~ /^\+\s*$logFunctions\s*\(\s*(?:(KERN_\S+\s*|[^"]*))?(?:_\(\s*)?"[X\t]*"(?:\s*\))?\s*(?:|,|\)\s*;)\s*$/ ||
		    $line =~ /^\+\s*(?:_\(\s*)?"[^"]*"(?:\s*\))?\s*(?:\s*|,|\)\s*;)\s*$/) &&
